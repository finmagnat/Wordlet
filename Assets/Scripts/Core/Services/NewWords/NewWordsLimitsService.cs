...
{
    public sealed class NewWordsLimitsService : IService, INewWordsLimitsService
    {
        ...
        public NewWordsSubmitAvailability GetAvailability()
        {
            int cooldownRemain = 0;
            bool dailyReached = false;
            int dailyResetRemain = 0;

            int dailyLimit = _game.newWordsDailyLimit;
            int cooldownSeconds = _game.newWordsCooldownSeconds;
            ...
            if (dailyLimit > 0)
            {
                ...
                if (usedToday >= dailyLimit)
                {
                    ...
                    return new NewWordsSubmitAvailability(
                        canSubmit: false,
        ...
        {
            if (_game.newWordsDailyLimit > 0)
                IncDailyCount();

            if (_game.newWordsCooldownSeconds > 0)
